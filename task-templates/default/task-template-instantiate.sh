...
function remove_language {
	...
	py_regex_replace "${sol_pattern}" "" "./solutions.json"
	rm -rf "./grader/${lang}"
	rm -rf "./public/"*"/${lang}"
	local pub_pattern
...
function handle_secret_tag {
	...
	py_regex_replace "${line_pattern}" "${replacement}" "validator/"* "checker/"*
	if "${has_grader}"; then
		py_regex_replace "${line_pattern}" "${replacement}" "grader/cpp/"*
		if "${has_java}"; then
			py_regex_replace "${line_pattern}" "${replacement}" "grader/java/"*
		fi
		if "${has_python}"; then
			printf -v "line_pattern" '#%s\\$%s%s\\n([^\\n]*\\n)' "${hws}" "${tag}" "${hws}"
			py_regex_replace "${line_pattern}" "${replacement}" "grader/py/"*
		fi
	fi
}
